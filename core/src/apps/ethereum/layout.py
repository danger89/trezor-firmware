...
def require_confirm_fee(
    ...
    return confirm_total_ethereum(
        ...
        format_ethereum_amount(gas_price, None, network),
        format_ethereum_amount(gas_price * gas_limit, None, network),
    )
...
async def require_confirm_eip1559_fee(
    ...
    await confirm_amount(
        ...
        amount=format_ethereum_amount(max_gas_fee, None, network),
    )
    await confirm_amount(
        ctx,
        title="Confirm fee",
        description="Priority fee per gas",
        amount=format_ethereum_amount(max_priority_fee, None, network),
    )
    await confirm_total(
        ctx,
        total_amount=format_ethereum_amount(spending, token, network),
        fee_amount=format_ethereum_amount(max_gas_fee * gas_limit, None, network),
