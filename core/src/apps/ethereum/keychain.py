...
if TYPE_CHECKING:
    ...
    HandlerWithKeychainAndNetworkFromPath = Callable[
        [wire.Context, MsgInKeychainNetworkPath, Keychain, EthereumNetworkInfo],
        Awaitable[MsgOut],
    ]

    HandlerWithKeychainAndDefsFromPath = Callable[
        [wire.Context, EthereumSignTypedData, Keychain, definitions.Definitions],
        Awaitable[MsgOut],
    ]
    ...
    HandlerWithKeychainAndDefsFromChainId = Callable[
        [wire.Context, MsgInKeychainChainId, Keychain, definitions.Definitions],
        Awaitable[MsgOut],
    ]
...
def with_keychain_and_network_from_path(
    *patterns: str,
    ...
    def decorator(
        ...
        async def wrapper(ctx: wire.Context, msg: MsgInKeychainNetworkPath) -> MsgOut:
...
def with_keychain_and_defs_from_chain_id(
    ...
    async def wrapper(ctx: wire.Context, msg: MsgInKeychainChainId) -> MsgOut:
        defs = definitions.get_definitions_from_msg(msg)
